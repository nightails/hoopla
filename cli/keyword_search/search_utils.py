...
from .index import BM25_B, BM25_K1, InvertedIndex
...
def normalize_single_term(term: str) -> str:
    tokens = process_text(term)

    if len(tokens) != 1:
        raise ValueError("Input must contain exactly one searchable term.")
...
def bm25search(term: str, limit=5) -> list[tuple[dict, float]]:
    movies_index = load_index()
    return movies_index.bm25_search(term, limit)


def bm25_idf_command(term: str) -> float:
    term = normalize_single_term(term)
    movies_index = load_index()
